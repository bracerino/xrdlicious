...
def _vesta_lattice(lattice):
    """
    Return a pymatgen Lattice in VESTA's Cartesian convention:
      a along X, b in XY plane, c determined by angles.
    This ensures our orientation matrix and rotation computation match VESTA
    regardless of how pymatgen loaded the original structure from CIF.
    """
    a, b, c = lattice.abc
...
def _vesta_lattice(lattice):
    ...
    Return a pymatgen Lattice in VESTA's Cartesian convention:
      a along X, b in XY plane, c determined by angles.
    This must be built manually because pymatgen's from_parameters uses a
    different convention (c along Z) in the version deployed here.
    """
    import math
    a, b, c = lattice.abc
