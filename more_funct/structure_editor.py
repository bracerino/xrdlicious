...
def _orientation_controls(key_suffix, lattice_matrix=None):
    ...
    for col, (label, p_mode, p_uvw, p_hkl) in zip(preset_cols, PRESETS):
        if col.button(label, key=f"orient_preset_{label}_{key_suffix}", use_container_width=True):
            ...
            # Safe to write — setdefault above already created these keys,
            # so Streamlit won't complain about "created with default + set via API".
...
def _render_py3dmol(atoms, structure, base_atom_size, show_lattice_vectors,
    ...
    # Normalise to VESTA convention (a along X, b in XY) so orientation matrix
    # and atom positions share the same Cartesian frame as VESTA.
...
def run_structure_editor(uploaded_files):
    ...
    with tab_viz:
        ...
        # Build asymmetric-unit atom list if requested
        if show_asym:
            try:
